...
class BenchmarkCNN(object):
  ...
  def __init__(self, params, dataset=None, model=None):
    ...
    subset = 'val' if params.eval else 'train'
    ...
    if self.mode in (constants.BenchmarkMode.EVAL,
      ...
      print("batch size {}, num devices {}, cpu_device {}, raw_devices {} \n".format(self.batch_size, len(self.devices), self.cpu_device, self.params,
      self.raw_devices))
      ...
      with tf.Session(
        ...
        try:
          ...
          if self.params.single_eval_device:
              self._eval_once_hvd(sess, summary_writer, graph_info.fetches,
                              graph_info.summary_op, image_producer, global_step)
          else: self._eval_once(sess, summary_writer, graph_info.fetches,
                          graph_info.summary_op, image_producer, global_step)
        if image_producer is not None:
          image_producer.done()
  ...
  def _eval_once_hvd(self, sess, summary_writer, fetches, summary_op,
  ...
  def _eval_once(self, sess, summary_writer, fetches, summary_op,
    ...
    with self._do_eval():
      ...
      if self.params.variable_update == 'horovod':
          import horovod.tensorflow as hvd
          if hvd.rank() == 0: log_fn('Accuracy @ 1 = %.4f Accuracy @ 5 = %.4f [%d examples]' %
                 (accuracy_at_1, accuracy_at_5, total_eval_count))
      else: log_fn('Accuracy @ 1 = %.4f Accuracy @ 5 = %.4f [%d examples]' %
             (accuracy_at_1, accuracy_at_5, total_eval_count))
      #--------------End----------------------------------------
      elapsed_time = loop_end_time - loop_start_time
      images_per_sec = (self.num_batches * self.batch_size / elapsed_time)
  ...
  def benchmark_with_session(self, sess, supervisor, graph_info,
    ...
    while not done_fn():
      ...
      if (eval_graph_info and local_step > 0 and not done_fn() and
        ...
        accuracy_at_1, accuracy_at_5 = None, None
        if self.params.single_eval_device:
            accuracy_at_1, accuracy_at_5 = self._eval_once_hvd(
                sess, summary_writer, eval_graph_info.fetches,
                eval_graph_info.summary_op, eval_image_producer,
                python_global_step)
        else: accuracy_at_1, accuracy_at_5 = self._eval_once(
            sess, summary_writer, eval_graph_info.fetches,
            eval_graph_info.summary_op, eval_image_producer,
            python_global_step)
        #---------------------End-----------------------------
  ...
  def _build_input_processing(self, shift_ratio=0):
    ...
    if self._doing_eval:
      ...
      mlperf.logger.log(key=mlperf.tags.PREPROC_NUM_EVAL_EXAMPLES,
                        value=self.dataset.num_examples_per_epoch('val'))
    else:
  ...
  def apply_larc(self, gradvars, learning_rate, eta=0.013, epsilon=1.0):
        ...
        true_vals = tf.scalar_mul(eta, tf.div(v_norms, g_norms))
        false_vals = tf.fill(tf.shape(v_norms), epsilon)
        larc_local_lr = tf.where(cond, true_vals, false_vals)

        ones = tf.ones_like(v_norms)
        lr = tf.fill(tf.shape(v_norms), learning_rate)
...
#        larc_grads = clipped_grads
        ...
        with tf.name_scope('append_apply_gradient_ops'):
          self.variable_mgr.append_apply_gradients_ops(
              gradient_state, opt, larc_grads, training_ops,
              loss_scale_params)
        #End---------------------------------------------------------------------
  ...
  def add_forward_pass_and_gradients(self,
    ...
    subset = 'val' if self._doing_eval else 'train'
