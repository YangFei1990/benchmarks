...
#--------------------Fei: modified learning rate from can's code
  def get_learning_rate(self, global_step, batch_size):
    def get_lr(lr, warmup_it, decay_steps, global_step,
    ...
    lc_beta = 0.00001
    lc_periods = 0.47
    decay_steps = 118287 * 100 // batch_size
    nstep_per_epoch = 118287 // batch_size
    warmup_it = 5 * nstep_per_epoch

    learning_rate = tf.cond(global_step < warmup_it,
            lambda: warmup_decay(warmup_lr, global_step, warmup_it,
                                 lr),
            lambda: get_lr(lr, warmup_it, decay_steps, global_step,
                           lc_periods, lc_alpha, lc_beta))
  ...
  def get_learning_rate_torch(self, global_step, batch_size, N_gpu):
    print("Batch size: {}, number GPUs: {}\n".format(batch_size, N_gpu))
    global_batch_size = (N_gpu * batch_size)
...
#--------------------------------End-------------
  ...
  def postprocess(self, results, using_hvd):
